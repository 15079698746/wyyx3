...
function init() {
  ...
  var maxImg = document.querySelector('.maxbox img'); // var detailcon=document.querySelector('.detailcon')
  ...
  minBox.onmousemove = function (ev) {
    ...
    console.log(offset(minBox).top); //2.2限制mask的移动范围
  ...
  for (var i = 0; i < showImg.length; i++) {
    showImg[i].onmouseover = function (ev) {
      var e = ev || window.event; //  console.log(e.target);
      //  console.log(minImg.src);
